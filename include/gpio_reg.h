...
static void gpio_init(void) {
    ...
    BTN_LEFT_DDR  &= ~(1 << BTN_LEFT_BIT); // Set left button pin (PD4) as input
    BTN_LEFT_PORT &= ~(1 << BTN_LEFT_BIT); // Disable pull-up resistor on left button pin (PD4) since the board has an external pull-down resistor
    BTN_RIGHT_DDR &= ~(1 << BTN_RIGHT_BIT); // Set right button pin (PD5) as input
    BTN_RIGHT_PORT&= ~(1 << BTN_RIGHT_BIT); // Disable pull-up resistor on right button pin (PD5) since the board has an external pull-down resistor

    // Red LED: output, off
    LED_DDR  |=  (1 << LED_BIT); // Set red LED pin (PD6) as output
    LED_PORT &= ~(1 << LED_BIT); // Ensure red LED is off by default
...
static bool btn_left_pressed(void) { // Check if the left button was just pressed (transition from not pressed to pressed)
    static bool prev = false; // Track the previous state of the left button to detect transitions
    bool now = btn_left_raw(); // Read the current state of the left button
...
static inline void led_on(void)     { LED_PORT |=  (1 << LED_BIT); } // Turn on the red LED by setting the corresponding bit in the PORT register
static inline void led_off(void)    { LED_PORT &= ~(1 << LED_BIT); } // Turn off the red LED by clearing the corresponding bit in the PORT register
static inline void led_toggle(void) { LED_PORT ^=  (1 << LED_BIT); } // Toggle the red LED by XORing the corresponding bit in the PORT register (if it's on, it will turn off; if it's off, it will turn on)
...
static void neo_flash(uint32_t color, uint16_t ms) { // Flash all NeoPixels with the specified color for a certain duration (ms milliseconds)
    neo_fill(color); // Fill all NeoPixels with the specified color to create the flash effect
    neo_show();
    for (uint16_t i = 0; i < ms / 10; i++) _delay_ms(10); // Wait for the specified duration while the NeoPixels are lit up
    neo_clear();
}
...
static void neo_show_progress(uint8_t done, uint32_t color) {
    ...
    for (uint8_t g = 0; g < done && g < NUM_GESTURES; g++) { // For each completed gesture
        for (uint8_t p = g * 3; p < (g * 3) + 3 && p < NUM_PIXELS; p++) { // Light up 3 NeoPixels for each completed gesture to visually indicate progress. For example, if gesture 0 is completed, light up pixels 0, 1, 2
...
static void neo_capturing_tick(uint32_t color, uint8_t gn) {
    CircuitPlayground.strip.clear();
    uint8_t start = gn * 3; // Each gesture corresponds to a group of 3 NeoPixels

    for (uint8_t p = start; p < start + 3 && p < NUM_PIXELS; p++) { // Light up the NeoPixels corresponding to the current gesture being captured to provide visual feedback during the capture process. For example, if we're capturing gesture 1, light up pixels 0, 1, 2
        neo_set(p, COLOR_WHITE); // Set the color of the NeoPixels corresponding to the current gesture being captured to white
    }
...
// Animations
static void neo_success_animation(void) { // Success animation: all NeoPixels flash green briefly and the red LED turns on to indicate a successful action (e.g. successful unlock)
...
static void neo_fail_animation(void) { // Failure animation: all NeoPixels flash red 3 times to indicate a failed action (e.g. failed unlock attempt)
    neo_flash(COLOR_RED, 400); _delay_ms(300);
    neo_flash(COLOR_RED, 400); _delay_ms(300);
    neo_flash(COLOR_RED, 400);
}

static void neo_saved_animation(void) { // Saved animation: all NeoPixels flash green once to indicate that a new key has been successfully saved during the recording process
    neo_flash(COLOR_GREEN, 500);
}

static void neo_erase_animation(void) { // Erase animation: all NeoPixels flash red once to indicate that the stored key has been erased
...
static void neo_boot_animation(void) { // Boot animation: all NeoPixels flash cyan to indicate that the system is booting up
